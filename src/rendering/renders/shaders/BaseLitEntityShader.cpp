...
void BaseLitEntityShader::get_uniforms_set_bindings() {
    ...
    texture_scale_location = get_uniform_location("texture_scale"); // added
...
void BaseLitEntityShader::set_instance_data(const BaseLitEntityInstanceData& instance_data) {
    ...
    glProgramUniform1fv(id(), texture_scale_location, 1, &entity_material.texture_scale); // added
}
...
void BaseLitEntityShader::set_point_lights(const std::vector<PointLight>& point_lights) {
    ...
    set_frag_define("NUM_PL", Formatter() << count); // changed: set_vert_define -> set_frag_define
